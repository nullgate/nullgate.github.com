...
(function()
{
	...
	Sphinx.options = { enable_blues: true, enable_remix: true, enable_prefetch: false};
	...
	function driver_resolve(schema, variant)
	{
		...
		var locator = "/sphinx/olap/" + schema + "/" + variant;
		if(Sphinx.options["enable_remix"])
		{
			Sphinx.latestLocator = schema + "/" + variant;
		}
		return locator;
	}
	...
	Sphinx.cache = function(eligble)
	{
		...
		for(var schema in cached_root)
		{
			...
			for(var variant in cached_list)
			{
				...
				var view_node = { "name" : decodeURI(canonical), "size": cached_list[variant]["_"]["volume"], "variant": canonical,"schema":schema };
